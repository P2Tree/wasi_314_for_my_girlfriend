...
% 现在我要把它改成二维的，修改成功
% 现在将它变成函数，修改成功
% 现在我要加入ekf更新粒子，修改为ekpf算法，修改成功
% 现在我要加入ukf更新粒子，改成upf
function [x_est_out error rms]=upf(Rs, rs, T, N, q, r)
...
%         z_update(:,i) = x_P_update(:,i);
        % 对第i个粒子在整个时间轴上滤波，也就是假设上边生成的粒子为观测量输入到ukf
        [zupf, pupf] = ukf3(f, initx, pupf, h, z_update(:,i), Q, R);
        % 生成每个粒子的权重，权重更新时一种概率模型，但这里我们知道我们的观测误差是高斯分布，协方差为x_R
        P_w(:,i) = (1 / sqrt(2 * pi * r)) * exp(- (z - zupf).^2 / (2 * r)); 
%         P_w(:,i) = P_w(:,i) .* (mean(zupf,2)) + sqrt(pupf) * randn(2, 1);
    end
    
    % 判断粒子退化
    Psum = sum(P_w,2);
    if Psum(1) == 0 || Psum(2) == 0
        disp('upf算法，粒子退化，无法运算');
        break;
    end
    % 规范化粒子权重，使和为1
    for a = 1:D
...
%     x_est_out(:,t) =x_est_out(:,t)  - 0.01*randn(2,1);
end
...
% legend('true position', 'mkpf estimate', 'measurement position');
% 
% figure(12);
% hold on;
